...
class BasicModel(nn.Module):
    ...
    def forward(self, data, prompt_points, epoch, *args, **kwargs): # 
        backbones_out = self.backbone(data)
        decoders_out = self.decoder(backbones_out, prompt_points, epoch) # 
        # decoders_out = self.decoder(backbones_out)
        return decoders_out
...
class SegDetectorModel(nn.Module):
    ...
    def forward(self, batch, epoch=0, training=True):
        if isinstance(batch, dict):
            ...
            prompt_points = batch['center_points'].to(self.device) # gs_image
        else:
            data = batch.to(self.device)
        data = data.float()
        pred = self.model(data, prompt_points, epoch, training=self.training)
